import {exitBlockers,hiddenCar} from './puzzle.js?v=47';
import {SoundEffects,EFFECT_NAMES,bindInteractionSounds} from './sfx.js?v=47';
import {pendingCars,garageStatus,routeOptions,chooseRoute} from './garage.js?v=47';
import {MusicPlayer,TRACKS} from './music.js?v=47';
import {LEVELS_PER_DISTRICT,districtFor,stageProfile} from './campaign.js?v=47';
import {Market,MARKET_LEVELS,DISTRICTS,STALL_ICONS} from './market.js?v=47';
import {MarketScene} from './market-scene.js?v=47';
import {LobbyScene} from './lobby-scene.js?v=47';
import {carPose,BAY,CAPACITY} from './traffic.js?v=47';
import {canExit,COLORS,LEVELS} from './game.js?v=47';
import {FLEET_STYLES,REGION_FLEETS,fleetFor,fleetStyles,vehicleLabel,styleDebut} from './fleet.js?v=47';
import {platform} from './platform.js?v=47';
import {normalize,complete,buyTheme,claimMission,MISSIONS,THEMES,TOTAL_LEVELS,marketGrowth} from './progress.js?v=47';
import {checkpoint,restoreSession} from './session.js?v=47';
...
const headings={shop:['MAKE IT YOURS','등불 상점','모은 코인으로 밤의 색을 바꿔 보세요.'],map:['YOUR NIGHT JOURNEY','오늘은 어느 골목으로?','10개 지역, 100번의 밤을 한 단계씩 여행해요.'],garage:['MEET YOUR SHUTTLES','지역별 셔틀 도감','10개의 거리, 20가지 차량. 다음 골목에서 만날 셔틀을 구경하세요.'],records:['YOUR LITTLE ACHIEVEMENTS','우리의 운행 기록','한 번의 출발이 모여 야시장을 밝힙니다.']};
...
function frame(now){
 ...
 if(view==='game'&&market){
  ...
  const board=market.state.cars.map(c=>c.id+':'+!!c.revealed).join(',')+':'+(market.state.puzzle?.gates||[]).map(g=>g.open).join(',');if(board!==boardSignature){boardSignature=board;targets();}renderGarage();renderCombo(now);renderEmergency();
  scene.colorAssist=save.colorAssist;scene.reduceMotion=reduced.matches;scene.decoration=save.decoration;scene.draw(market,selected);renderQueue();
...
function puzzleTrials(){
 showModal('새로운 길을 미리 만나세요',`<p>기록이나 진행 단계를 바꾸지 않는 무료 체험이에요.</p><div class="puzzle-trials"><button data-puzzle-trial="20">21단계 · 가려진 셔틀</button><button data-puzzle-trial="40">41단계 · 열쇠와 차단기</button><button data-puzzle-trial="70">71단계 · 장치 조합</button></div>`,'닫기',hideModal);
 document.querySelectorAll('[data-puzzle-trial]').forEach(b=>b.onclick=()=>{if(!run?.practice)practiceReturn={market,run,finished};start(Number(b.dataset.puzzleTrial),true);});
}
