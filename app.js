import {claimEventReward,EVENT_REWARDS} from './mission-rewards.js?v=53';
import {ITEMS,BUNDLE,purchase,useItem,itemAvailability,navigationSuggestion,queueGroups} from './items.js?v=53';
import {cutActive} from './queue-cut.js?v=53';
import {exitBlockers,hiddenCar} from './puzzle.js?v=53';
import {SoundEffects,EFFECT_NAMES,bindInteractionSounds} from './sfx.js?v=53';
import {pendingCars,garageStatus,routeOptions,chooseRoute} from './garage.js?v=53';
import {MusicPlayer,TRACKS} from './music.js?v=53';
import {LEVELS_PER_DISTRICT,districtFor,stageProfile} from './campaign.js?v=53';
import {Market,MARKET_LEVELS,DISTRICTS,STALL_ICONS} from './market.js?v=53';
import {MarketScene} from './market-scene.js?v=53';
import {LobbyScene} from './lobby-scene.js?v=53';
import {carPose,BAY,CAPACITY} from './traffic.js?v=53';
import {canExit,COLORS,LEVELS} from './game.js?v=53';
import {FLEET_STYLES,REGION_FLEETS,fleetFor,fleetStyles,vehicleLabel,styleDebut} from './fleet.js?v=53';
import {platform} from './platform.js?v=53';
import {normalize,complete,buyTheme,claimMission,MISSIONS,THEMES,TOTAL_LEVELS,marketGrowth} from './progress.js?v=53';
import {checkpoint,restoreSession} from './session.js?v=53';
...
const headings={shop:['READY FOR THE RUSH','셔틀 보급소','모은 코인으로 막힌 순간을 풀어 줄 아이템을 준비하세요.'],map:['YOUR NIGHT JOURNEY','오늘은 어느 골목으로?','10개 지역, 100번의 밤을 한 단계씩 여행해요.'],garage:['MEET YOUR SHUTTLES','지역별 셔틀 도감','10개의 거리, 20가지 차량. 다음 골목에서 만날 셔틀을 구경하세요.'],records:['YOUR LITTLE ACHIEVEMENTS','우리의 운행 기록','한 번의 출발이 모여 야시장을 밝힙니다.']};
...
function activateItem(id){
 ...
 finished=false;targets();hideModal();sfx.play(id==='bay'?'expand':'hint');renderBagBadge();
...
function expandOptions(){
 ...
 showModal('승강장 한 칸 더',`<p>이번 운행의 승강장을 ${market.state.bays.length+1}칸으로 늘려요.</p><p class="subtle">확장권 보유 ${save.inventory.bay}개 · 최대 7칸<br>확장해서 완료하면 별 1개를 받아요.</p><button id="expand-ad" class="secondary">${window.PickupRushAds?.showRewarded?'광고 보고 열기':'무료 테스트 광고로 열기'}</button>`,save.inventory.bay?'확장권 1개 사용':`확장권 구매 · ${ITEMS.find(i=>i.id==='bay').price}코인`,()=>{if(save.inventory.bay)activateItem('bay');else confirmPurchase('bay',expandOptions);});$('#expand-ad').onclick=reward;
}
