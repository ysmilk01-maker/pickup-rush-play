import {timeTargets,formatTime,timerState} from './timing.js?v=55';
import {claimEventReward,EVENT_REWARDS} from './mission-rewards.js?v=55';
import {ITEMS,BUNDLE,purchase,useItem,itemAvailability,navigationSuggestion,queueGroups} from './items.js?v=55';
import {cutActive} from './queue-cut.js?v=55';
import {exitBlockers,hiddenCar} from './puzzle.js?v=55';
import {SoundEffects,EFFECT_NAMES,bindInteractionSounds} from './sfx.js?v=55';
import {pendingCars,garageStatus,routeOptions,chooseRoute} from './garage.js?v=55';
import {MusicPlayer,TRACKS} from './music.js?v=55';
import {LEVELS_PER_DISTRICT,districtFor,stageProfile} from './campaign.js?v=55';
import {Market,MARKET_LEVELS,DISTRICTS,STALL_ICONS} from './market.js?v=55';
import {MarketScene} from './market-scene.js?v=55';
import {LobbyScene} from './lobby-scene.js?v=55';
import {carPose,BAY,CAPACITY} from './traffic.js?v=55';
import {canExit,COLORS,LEVELS} from './game.js?v=55';
import {FLEET_STYLES,REGION_FLEETS,fleetFor,fleetStyles,vehicleLabel,styleDebut} from './fleet.js?v=55';
import {platform} from './platform.js?v=55';
import {normalize,complete,buyTheme,claimMission,MISSIONS,THEMES,TOTAL_LEVELS,marketGrowth} from './progress.js?v=55';
import {checkpoint,restoreSession} from './session.js?v=55';
...
function finish(){
 ...
 if(market.state.status==='won'){
  ...
  sfx.play('lose');showModal('승강장이 모두 찼어요','<p>줄 맨 앞 손님과 같은 색 셔틀이 없어요.<br>되돌려서 필요한 차가 들어올 자리를 만들어 보세요.</p><button id="lost-undo" class="secondary">되돌리기 · 100코인</button><button id="lost-ad" class="secondary">확장권 / 광고로 승강장 +1</button><button id="lost-home" class="secondary">대기실로 돌아가기</button>','무료 다시 도전',()=>start(market.state.levelIndex,!!run.practice));$('#lost-undo').disabled=!!itemAvailability(market,run,'undo');$('#lost-undo').textContent=save.inventory.undo?`되돌리기 · 보유 ${save.inventory.undo}개`:'되돌리기 · 100코인';$('#lost-undo').onclick=undoOptions;$('#lost-ad').disabled=market.state.bays.length>=7;$('#lost-ad').onclick=expandOptions;$('#lost-home').onclick=()=>showLobby('home');
 }
}
let lobbyFrame=0;
function frame(now){
...
const itemMessages={'no-undo':'되돌릴 배차 기록이 없어요. 아이템은 사용되지 않았어요.','shuffle-unavailable':'지금은 안전하게 섞을 차량이 부족해요. 아이템은 사용되지 않았어요.',coins:'코인이 부족해요. 운행과 미션으로 모아 보세요.',limit:'아이템은 종류별로 최대 99개까지 보관할 수 있어요.',storage:'저장하지 못해 거래를 취소했어요. 코인과 아이템은 그대로예요.',practice:'무료 체험에서는 보유 아이템을 사용하지 않아요.',busy:'셔틀 주행과 탑승이 끝난 뒤 가방을 열어 주세요.',max:'승강장은 최대 7칸이에요.',active:'이번 운행에 이미 적용 중이에요.',lost:'승강장 확장이나 되돌리기로 운행을 재개해 주세요.',empty:'보유 아이템이 없어요.',ended:'완료한 운행에는 사용할 수 없어요.',start:'운행을 시작한 뒤 사용해 주세요.'};
