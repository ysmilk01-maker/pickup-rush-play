import { Traffic, carPose, BAY } from './traffic.js?v=10';
import { Scene } from './scene.js?v=10';
import { LEVELS, COLORS, VEHICLE_TYPES, canExit } from './game.js?v=10';
import { platform } from './platform.js?v=10';
...
$('#settings').onclick=()=>{open('픽업 러시',`<p>화살표 앞이 열린 차량을 보내세요.<br>같은 색 승객이 타고, 만차가 되면 출발합니다.</p><select id="level-picker" aria-label="스테이지 선택">${LEVELS.map((l,i)=>`<option value="${i}" ${i===traffic.state.levelIndex?'selected':''}>레벨 ${i+1} · ${l.title}</option>`).join('')}</select><div class="settings-row"><span>택시 4석 · 봉고차 6석 · 버스 10석</span></div>`,'선택한 레벨 시작','settings');};
...
$('#sort').onclick=()=>{if(!sorts){toast('이번 레벨의 정렬을 사용했습니다');return;}if(traffic.walkers.length){toast('승객 탑승이 끝난 뒤 사용해 주세요');return;}const parked=traffic.state.bays.find(c=>c?.phase==='parked'&&c.loaded+c.pending<c.capacity);if(!parked){toast('대기 중인 차량이 있을 때 사용할 수 있어요');return;}const color=parked.color;traffic.state.queue=[...traffic.state.queue.filter(c=>c===color),...traffic.state.queue.filter(c=>c!==color)];sorts--;traffic.state.status='playing';finished=false;targets();toast('대기 차량과 같은 색 승객을 앞으로 보냈습니다');};
