...
export function passengerQueue(cars,level,capacity){
  const random=seededRandom(7301+level*104729),queue=[];
  let offset=0,wave=0;
  while(offset<cars.length){
    const width=level<3?2:level<12?3:level<24?(wave%3===0?4:3):4;
    ...
    while(remaining.size){
      ...
      const amount=Math.min(remaining.get(color),1+Math.floor(random()*3));
