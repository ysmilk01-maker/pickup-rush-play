import {canExit} from './game.js?v=50';
import {hiddenCar} from './puzzle.js?v=50';

export const CUT_LIMIT=3;
export const cutCount=level=>level<30?2:3;
export const cutActive=q=>q?.status==='entering'||q?.status==='active';
...
export function beginQueueCut(t){
 ...
 const original=s.people.map(p=>p.id),chosen=new Set(indices),pairs=s.queue.map((color,i)=>({color,person:s.people[i]}));
 const guests=indices.map(i=>pairs[i]),reordered=[...guests,...pairs.filter((_,i)=>!chosen.has(i))];
 s.queue=reordered.map(p=>p.color);s.people=reordered.map(p=>p.person);
 s.queueCut={color:car.color,ids:guests.map(p=>p.person.id),originOrder:original,boarded:[],targetCar:car.id,
  limit:CUT_LIMIT,remaining:CUT_LIMIT,status:'entering',elapsed:0,announced:false,resultSeen:false};
 t.queueVisual=t.queueConsumed;t.onEvent?.({type:'cutin'});return true;
}
...
export function validQueueCut(q,m,s){
 if(!q)return true;
 if(m.demandVersion<8||s.levelIndex<4||s.puzzle?.tutorial||!['active','success','missed'].includes(q.status)||q.elapsed!==.9||q.limit!==CUT_LIMIT||!Number.isInteger(q.remaining)||q.remaining<0||q.remaining>CUT_LIMIT||typeof q.announced!=='boolean'||typeof q.resultSeen!=='boolean')return false;
 ...
 if(q.status!=='missed'&&q.ids.some(id=>q.boarded.includes(id)===waiting.has(id)))return false;
 ...
 if(q.status==='active'&&s.emergency?.status==='active')return false;
 return true;
}
