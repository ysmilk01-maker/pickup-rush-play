...
export const EFFECT_NAMES={cutin:'새치기 등장',cutSuccess:'새치기 손님 탑승',cutReturn:'새치기 줄 복귀',reveal:'셔틀 색 공개',gate:'차단기 개방',ui:'메뉴 선택',close:'창 닫기',start:'운행 시작',resume:'운행 재개',pause:'일시정지',select:'차량 출발',blocked:'길 막힘',arrival:'승강장 도착',walk:'승객 발걸음',board:'승객 탑승',depart:'만차 출발',incoming:'차고 입차',parked:'차고 주차 완료',undo:'되돌리기',hint:'길 찾기',reward:'코인 보상',expand:'승강장 확장',notice:'사용 불가',warning:'긴급 운행 임박',success:'긴급 운행 성공',timeout:'긴급 운행 종료',combo:'연속 만차',clear:'클리어',lose:'승강장 가득 참',emergency:'응급차 사이렌'};
const LIMITS={cutin:.6,cutSuccess:.6,cutReturn:.6,reveal:.22,gate:.22,ui:.07,close:.1,start:.4,resume:.2,pause:.2,select:.08,blocked:.22,arrival:.16,walk:.16,board:.07,depart:.18,incoming:.5,parked:.3,undo:.2,hint:.25,reward:.4,expand:.5,notice:.3,warning:.4,success:.6,timeout:.6,combo:.25,clear:1,lose:.5,emergency:2};
...
export class SoundEffects{
 ...
 play(name){
  ...
  try{
   switch(name){
    case 'cutin':this.hiss(.15,.045,1800);this.tone(440,.16,.045,0,'triangle',880);this.tone(1047,.13,.04,.16);this.tone(784,.2,.04,.29);break;
    case 'cutSuccess':[784,1047,1319].forEach((n,i)=>this.tone(n,.2,.045,i*.08));break;
    case 'cutReturn':this.tone(659,.14,.035,0,'sine',440);this.tone(392,.22,.03,.14);break;
