import {Market} from './market.js?v=55';
import {MarketScene} from './market-scene.js?v=55';
import {COLORS,canExit} from './game.js?v=55';
import {CAPACITY} from './traffic.js?v=55';
import {pendingCars} from './garage.js?v=55';
import {stageProfile} from './campaign.js?v=55';
import {overlaps,bounds,LOT} from './geometry.js?v=55';
...
function load(index){
 index=Math.max(0,Math.min(199,index));m=new Market(index);auto=false;started=false;custom=false;cursor=0;selection=null;events=[];
 $('#level').value=index;$('#previous').disabled=index===0;$('#next').disabled=index===199;$('#auto').textContent='해법 재생';
